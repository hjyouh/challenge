(function () {
  ...
  if (document.body.getAttribute('data-page') !== 'auth') {
    ...
    if (!isStandalone && !localStorage.getItem('pwa-popup-nodisplay')) {
      ...
      pwaOv.innerHTML = [
        '<div style="background:#1e1c1a;border:1px solid rgba(180,176,168,0.3);border-radius:16px;padding:28px 24px 0;width:min(100%,380px);font-family:SUITE,Poppins,Arial,sans-serif;color:#f8fafc;text-align:center;">',
        '<p style="font-size:18px;font-weight:800;color:#22c55e;line-height:1.5;margin:0 0 16px">천사님 인스타 챌린지 앱에<br/>오신 것을 환영합니다.</p>',
        '<p style="font-size:15px;color:#cbd5e1;line-height:1.7;margin:0 0 12px">앞으로 천사님 챌린지를<br/><strong style="color:#22c55e">홈 화면에 아이콘을 생성하시면</strong><br/>편리하게 사용하실 수 있습니다.</p>',
        '<p style="font-size:15px;color:#cbd5e1;line-height:1.7;margin:0 0 20px">홈화면에 아이콘 설치는<br/><strong style="color:#22c55e">주메뉴 &gt; 마이에 자세히 설명</strong>되어 있습니다.<br/><strong style="color:#22c55e">마이 메뉴에서 확인하고 설치</strong>해 주세요.</p>',
        ...
        '<div style="display:flex;border-top:1px solid rgba(180,176,168,0.2);margin:12px -24px 0;border-radius:0 0 16px 16px;overflow:hidden;">',
        '<button id="pwaLater" style="flex:1;background:none;border:none;border-right:1px solid rgba(180,176,168,0.2);color:#94a3b8;font-size:16px;padding:16px;cursor:pointer;">다음에</button>',
        '<button id="pwaGo" style="flex:1;background:none;border:none;color:#22c55e;font-size:16px;font-weight:800;padding:16px;cursor:pointer;">마이메뉴로 이동</button>',
        '</div></div>',
      ].join('');
